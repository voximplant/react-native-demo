...
const ConferenceScreen = ({route}: IScreenProps<'Conference'>) => {
  ...
  const {startConference, endConference, muteAudio, sendLocalVideo} =
    ConferenceService();
  const {dynamicComputeStyles} = useUtils();

  const isSendVideo = useSelector(
    (state: RootReducer) => state.conferenceReducer.sendLocalVideo,
  );
  const isMuted = useSelector(
    (state: RootReducer) => state.conferenceReducer.isMuted,
  );
  const callState = useSelector(
    (state: RootReducer) => state.conferenceReducer.callState,
  );
  const participants = useSelector(
    (state: RootReducer) => state.conferenceReducer.participants,
  );
